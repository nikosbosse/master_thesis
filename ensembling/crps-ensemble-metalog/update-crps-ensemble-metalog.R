# ============================================================================ #
# do ensembling with predictive samples from metalog
# ============================================================================ #
...
samples <- forecasts[, .(y_pred = get_samples_metalog(value, quantile, n_samples = n_samples),
...
if (!dir.exists(here::here("ensembling", "crps-ensemble-metalog", "crps-weights-metalog",
                           paste(settings$ensemble_past_included,
                                 settings$crps_optimisation_horizon, sep = "-")))) {
  dir.create(here::here("ensembling", "crps-ensemble-metalog", "crps-weights-metalog",
                        paste(settings$ensemble_past_included,
                              settings$crps_optimisation_horizon, sep = "-")))
}

filename <- here::here("ensembling", "crps-ensemble-metalog", "crps-weights-metalog",
                       paste(settings$ensemble_past_included,
                             settings$crps_optimisation_horizon, sep = "-"),
                       paste0(forecast_date,
                              "-crps-weights-metalog.csv"))
...
filename <- here::here("data", "processed-data",
                       paste("crps-ensemble-metalog",
                             settings$ensemble_past_included,
                             settings$crps_optimisation_horizon, sep = "-"),
                       paste0(forecast_date, "-crps-ensemble-metalog.csv"))
