...
Chapters \@ref(evaluation) and \@ref(model-aggregation) have laid out the tools necessary for model evaluation and model aggregation. Chapter \@ref(background-data) has introduced the data and the models. This chapter finally applies the tools to the data and analyses the performance of the Forecast Hub models as well as the ensembles in detail to find out which models perform well and why. 

The structure of this Chapter largely follows the general structure proposed in Chapter \@ref(evaluation), at times digressing a bit to discuss additional aspects of interest. The starting point of the analysis forms the visualisation of the forecasts and observed values. We then assess overall model performance to determine which models perform well and which less so. To that end we examine summarised scores and take a look at scores in terms of a mixed effects regression. This first analysis tells us which models perform well, but not why. In order to obtain an explanation, we subsequently analyse how the different metrics relate, what contributes to the performance measures and what drives differences in performance. We first examine correlations between the individual metrics. While this is not strictly necessary for the model evaluation, it provides us with a better understanding of the metrics we use. We then look at how different aspects of the predictive distribution contribute to different scores. This analysis focuses mainly on the weighted interval score (WIS) and breaks it down into its components, but we also look at how different ranges of the predictive distributions contribute to our measures of sharpness, calibration and overall performance. This again gives us a deeper understanding of the metrics, but is also a first step towards finding out what drives overall performance. We then look at the main external factors and especially the characteristics of the different locations that drive performance differences. After having analysed the properties of the metrics and external factors that drive differences in scores, we then look at the innate model properties that explain performance differences. We look at different aspects of calibration and sharpness in detail that help us to explain performance differences as well as hint to ways in which the models could be improved. The evaluation is followed by a more detailed look at the ensemble models where we discuss some aspects specific to the ensembles in more detail. These include a look at ensemble weights over time as well as analysis of different ensemble alternatives. Finally we conduct a small sensitivity analysis that serves to check the plausibility of the inferences made throughout the chapter. Finally, a short summary and discussion of limitations concludes this lenghty chapter. 
...
Table \@ref(tab:regression-ensemble) shows again a regression of the WIS on the different ensemble variants with the COVIDhub-ensemble model as the baseline. The regression results that we can indeed not distinguish the top performing qra-ensembles as well as the crps-ensembles optimised for two-week-ahead predictions from the COVIDhub-ensemble model. But again, we also see that none of the models is able to beat the COVIDhub-ensemble model. 
...
We also conducted a sensitivity analysis for the ensemble variants. Figure \@ref(fig:senitivity-esenmbles) shows aggregated scores for all ensemble variants for the four different scenarios. We can see that the qra-ensemble-4 stayed at the top consistently. At least in scenario for QRA outperformed the CRPS model aggregation approach, but the pattern overall is rather inconclusive. The comparison is also not fair insofar as the CRPS model aggregation approach also involved a loss of precision due to random sampling. 
