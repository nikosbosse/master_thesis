...
evaluation = what performs well and why? --> we start with the what. 

We now want to evaluate overall performance. This is the thing we are most interested in. It also as a mental anchor for the coming steps. The rest is about what drives performance. 

In order to evaluate performance, we first have a look at the aggregated scores from the different metrics. After that we perform a mixed effects regression that helps us to understand the ranking better and forms the basis for a model selection decision. Idea: we probably shouldn't choose a model that performs significantly worse than the top performer, but we should still look out for calibration and sharpness. 
...
## Understanding the metrics and what really drives them

We now have a model ranking. But we still want to understand the metrics better. 
This part is probably not strictly necessary for model evaluation, but we want to learn here. 
...
### Contributions to WIS from sharpness and over- / under-prediction

Question: what mainly drives WIS? 
...
### Contributions to WIS from different ranges
...
``` {r scores-ranges, echo = FALSE, out.width = "100%", fig.cap = "Story here: outer interval ranges are more wrong, but get weighted down"}
...
<!-- ``` {r sharpness-ranges, echo = FALSE, fig.show = "hold", out.width = "100%", fig.cap = "Sharpness across all states and forecast dates and horizons for different interval ranges"} -->



<!-- ``` -->

<!-- ``` {r penalty-ranges, echo = FALSE, out.width = "100%", fig.cap = "Interval scores across all states and forecast dates and horizons for different interval ranges"} -->
...
- not differences in forecast dates
...
a) understanding how the interval score works
b) where do models perform well / badly? 

We are interested in what drives performance. 

- correlation between WIS and metrics
...
```{r random-effects-model-code2, eval = FALSE}
fit <- lme4::lmer(log_scores ~ model + horizon + (1|state) + (1|forecast_date),
                  data = unsummarised_scores)
...
```{r random-effects-model2, message = FALSE, echo = FALSE, message=FALSE, results="as.is"}
library(kableExtra)
...
``` {r random-effects2, echo = FALSE, out.width = "100%", fig.cap = "Random effects of the different locations (left) and forecast dates (right"}
knitr::include_graphics("../visualisation/chapter-5-results/random-effects.png")
