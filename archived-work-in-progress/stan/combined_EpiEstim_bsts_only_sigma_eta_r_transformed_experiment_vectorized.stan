...
transformed data{
  ...
  int sampling_inc_tmp[tau, T];
  int sampling_inc[tau * T];
  infectiousness[1] = 1;
  ...
  for (i in 1:tau){
    sampling_inc_tmp[,i] = rep_array(1, tau);
  }

  for (s in (tau + 1):T){
    sampling_inc_tmp[,s] = past_incidences[(s-tau+1):s];
  }

  sampling_inc = to_array_1d(sampling_inc_tmp[,]);
...
transformed parameters{
  ...
  real expected_inc_tmp[tau, T];
  real expected_inc[tau * T];
  ...
  for (i in 1:tau){
    expected_inc_tmp[,i] = rep_array(1, tau);
  }
  for (s in (tau + 1):T){
    expected_inc_tmp[,s] = to_array_1d(infectiousness[(s-tau+1):s] * R[s]);
  }

  expected_inc = to_array_1d(expected_inc_tmp[]);
}
...
model {
  ...
  sampling_inc[(tau + 1):(tau * T)] ~ neg_binomial_2(expected_inc[(tau + 1):(tau * T)], disp);
