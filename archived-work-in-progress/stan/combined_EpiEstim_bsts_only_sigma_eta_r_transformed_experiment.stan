...
transformed data{
  ...
  for (s in 2:T){
    infectiousness[s] = 0.0001; // this should maybe be zero? or 0.0001
    for (i in 1:(s - 1)){
...
transformed parameters{
  ...
  vector[T] delta;
  real disp = 1/(disp_inv * 0.7)^2;
  real <lower = 0> R[T]; // transformed to fit in the correct range
  real r[T];  

  r[1] = 0;
  delta[1] = delta_shock[1] * sigma_eta * 5;
  R[1] = 15 * exp(r[1]) / (1 + exp(r[1]) );

  for (t in 1:(T-1)){
    delta[t+1] = D + 0.3 * phi * (delta[t] - D) + delta_shock[t + 1] * sigma_eta;
    r[t+1] = r[t] + delta[t+1];
    R[t+1] = 15 * exp(r[t+1]) / (1 + exp(r[t+1]) );
  }
}
...
/*  for (s in (tau + 1):T){
  ...
  /*delta_shock[1] ~ normal(D, 5 * sigma_eta); // set high prior to make first change easy
  for (s in 1:(T-1)){
    delta_shock[s + 1] ~ normal(D + 0.1 * phi * (delta[s] - D), sigma_eta); 
  }
  */
  delta_shock ~ normal(0,1);
