...
transformed data{
  ...
  real w[t + n_pred + 5]; // - 1 actually
  infectiousness[1] = 0;

  for (i in 1:(t + n_pred + 5)){
    if (i > 40){
      w[i] = 0;
    } else {
...
generated quantities{
  real I_pred[n_pred + 2];
  real infectiousness_pred[n_pred + 2];
  real r_pred[n_pred + 2];
  real delta_pred[n_pred + 2];
  real delta_t; 
  ...
  for (s in 2:n_pred){
    ...
    for (i in 1:(t)){
      infectiousness_pred[s] +=  past_incidences[i] * w[t + s - i]; //remove -1
    }  
